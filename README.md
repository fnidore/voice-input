...
- 💊 **录音胶囊浮窗** —— 聆听/识别/已粘贴实时状态，可按住拖到任意位置（自动记忆），不抢焦点
- 📝 **识别历史 + 热词增强** —— 专有名词识别更准
...
> - 🧮 原生包内置 **CPU 版 PyTorch**；有 **N 卡**的用户在 设置 →「基础」→「⚡ 启用 GPU 加速」**一键下载 CUDA 运行时**（约 3GB，重启生效），无需源码安装。
...
| Tab | 内容 |
|---|---|
| **基础** | 模型设备 (cuda:0/cpu)、**⚡ GPU 加速一键启用**（N 卡用户）、语言、输入方式、麦克风（带音量条测试按钮）、提示音、录音浮窗开关（含恢复默认位置）、开机自启；底栏显示版本与实际设备（如 `v0.3.5 · CUDA`） |
| **快捷键** | 自定义说话快捷键（点按钮捕获）、自定义粘贴键 |
| **热词** | 多行编辑，空格分隔，让识别更准（如 `涛涛鱼 SenseVoice 机械臂`） |
| **历史** | 最近 20 条识别结果，单击复制到剪贴板 |
...
**GPU 加速**：原生安装包内置 CPU 版 PyTorch（`cuda:0` 自动回退 cpu）。有 N 卡时设置页会出现「⚡ 启用 GPU 加速」按钮：一键下载 CUDA 运行时（约 3GB，国内镜像 + 断点续传 + sha256 校验），下载完成**重启应用**即走 GPU——底栏显示 `· CUDA` 即生效。不要了点「删除运行时」可释放约 6GB 磁盘。
...
- **平台支持**：🐧 Linux/X11 是一等公民（功能完整、已真机验证）；🪟 Windows / 🍎 macOS 为跨平台支持，目前**实验性**，欢迎反馈。
- **中文键盘输入**：原生安装包（打包版）里，设置窗口文本框**直接用键盘打中文受输入法插件限制**（fcitx5/ibus 的 Qt6 插件难随包分发），请**复制后 Ctrl+V 粘贴**。⚠️ 这**只影响设置框输入**，不影响核心功能——语音识别出的中文照常输出到任意应用。
- **GPU 加速**：原生包内置 **CPU 版 PyTorch**（`cuda:0` 会自动回退 cpu）；N 卡用户在设置页「⚡ 启用 GPU 加速」一键下载 CUDA 运行时即可，无需源码安装。
- **模型下载**：首次运行/切换模型会联网从 ModelScope 下载（约 1GB）到 `~/.cache/modelscope`。
